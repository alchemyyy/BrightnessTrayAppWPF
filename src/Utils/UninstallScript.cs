...
public static class UninstallScript
{
    ...
    // Crucially, none of these match the user-data files that share the LocalAppData install dir
    // (settings.xml, displays.json, btawpf-*.log, nightlight-rva-cache.json, symbols\*).
    ...
    private static string BuildScript(string installDir, WindowsUninstallRegistry.Scope regScope, bool deleteSettings)
    {
        ...
        else
        {
            ...
            foreach (string pattern in RuntimeFilePatterns) sb.AppendLine($"del /f /q \"{EscBat(installDir)}\\{pattern}\" >nul 2>&1");
