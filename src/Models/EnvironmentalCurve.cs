...
{
    // List properties default to empty - NOT to CreateDefault() - because XmlSerializer rehydrates
    // collection properties by calling the getter and Add()ing each <P> element, ignoring the setter.
    // A non-empty initializer would mean every Save->Load round trip stacks another copy of the default edge pair
    // on top of the saved data. EnsureNormalized backfills empties with defaults at use-time instead.
    [XmlArray("Brightness")]
    [XmlArrayItem("P")]
    ...
    [XmlAttribute] public double BrightnessOffsetMin { get; set; } = 0.0;
    [XmlAttribute] public double BrightnessOffsetMax { get; set; } = 100.0;
    ...
    [XmlAttribute] public bool FollowTheSun { get; set; } = true;

    // Combined anchor state for the brightness/night-light curve under FollowTheSun.
    // [XmlIgnore] because the XML shape is preserved by the four flat properties below
    ...
    // Default Date = DateTime.MinValue maps to the "" string sentinel on serialize.
    [XmlIgnore] public SunAnchor BrightnessAnchor { get; set; } = new(default, 0.0, 0.0, true);
    ...
    // Stored as string instead of DateTime to dodge XmlSerializer's timezone reinterpretation of date-only values.
    [XmlAttribute]
    ...
    [XmlAttribute] public bool UseDaylightSavings { get; set; } = true;
    ...
    // Per-profile.
    [XmlAttribute] public bool DisabledPeriodEnabled { get; set; } = false;
    [XmlAttribute] public double DisabledPeriodStart { get; set; } = 0.25;
    [XmlAttribute] public double DisabledPeriodEnd { get; set; } = 0.75;
    ...
    [XmlAttribute] public bool DisabledPeriodFollowTheSun { get; set; } = false;
    ...
    // [XmlIgnore] because the four flat LastDisabledPeriodSunShift* properties below preserve the XML shape.
    [XmlIgnore] public SunAnchor DisabledPeriodAnchor { get; set; } = new(default, 0.0, 0.0, true);
    ...
    /// Cleans up the historical XmlSerializer accumulation bug
    /// where the property initializers were appended-to rather than replaced on every deserialization.
    /// </summary>
    ...
    private static void CollapseEdge(List<EnvironmentalCurvePoint> series, double edgeTime)
    {
        ...
        foreach (EnvironmentalCurvePoint p in series)
            if (p.Time == edgeTime) keepValue = p.Value;
