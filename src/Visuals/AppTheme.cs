...
/// <summary>
/// Profile button configuration (count + per-index default glyphs + lookup helper).
/// </summary>
public class ProfileButtonSettings
{
    /// <summary>Number of profile buttons to display.</summary>
    [XmlAttribute]
    public int ButtonCount { get; set; } = 4;

    /// <summary>Default glyph strings for profile buttons indexed by position (1, 2, 3, ...).</summary>
    [XmlArray("DefaultGlyphs")]
    [XmlArrayItem("Glyph")]
    ...
    /// <summary>
    /// Returns the custom glyph if set, otherwise the default for the given index,
    /// falling back to the 1-based number as a string when the index exceeds the defaults list.
    /// </summary>
...
/// <summary>
/// Centralized theme: every color, glyph, and profile-button default lives directly here as instance state,
/// persisted to theme.xml.
...
public sealed class AppTheme : IDisposable
{
    ...
    /// <summary>Display-identifier overlay background.</summary>
    public ThemeColor DisplayIdentifierBackground { get; set; } = new("E6202020");

    /// <summary>Display-identifier overlay border.</summary>
    public ThemeColor DisplayIdentifierBorder { get; set; } = new("33FFFFFF");

    /// <summary>Display-identifier overlay drop-shadow color.</summary>
    public ThemeColor DisplayIdentifierShadow { get; set; } = new("000000");

    /// <summary>Display-identifier overlay foreground.</summary>
    public ThemeColor DisplayIdentifierForeground { get; set; } = new("FFFFFF");

    // ===========================================================================
    // Curve editor colors.
    // Same value for both light and dark variants so a fresh install looks identical regardless of system theme.
    // Backdrop entries carry their original alpha for fidelity.
    // ===========================================================================

    /// <summary>Environmental brightness curve color.</summary>
    public ThemeColor EnvironmentalBrightnessCurve { get; set; } = new("7B949B", "DCF3FA");

    /// <summary>Environmental night-light curve color.</summary>
    public ThemeColor EnvironmentalNightLightCurve { get; set; } = new("FDCB43", "fedb7c");

    /// <summary>Environmental current-time marker color.</summary>
    public ThemeColor EnvironmentalCurrentTime { get; set; } = new("4E4E4E", "ffffff");

    /// <summary>Environmental twilight backdrop band (alpha-aware).</summary>
    public ThemeColor EnvironmentalTwilightBackdrop { get; set; } = new("40FF8C00");

    /// <summary>Environmental night backdrop band (alpha-aware).</summary>
    public ThemeColor EnvironmentalNightBackdrop { get; set; } = new("404C5A78");

    /// <summary>Curve editor grid line color.</summary>
    public ThemeColor EnvironmentalGridLine { get; set; } = new("929292", "939393");

    /// <summary>Curve-editor disabled-period overlay (translucent white).</summary>
    public ThemeColor CurveDisabledBandOverlay { get; set; } = new("08FFFFFF");

    // ===========================================================================
    // Glyphs (Segoe Fluent Icons codepoints)
    // ===========================================================================

    /// <summary>Monitor icon for individual displays.</summary>
    ...
    /// <summary>Display settings icon (DeviceLaptopNoPic).</summary>
    ...
    /// <summary>Save profile icon.</summary>
    public string GlyphProfileSave { get; set; } = GlyphCatalog.PROFILE_SAVE;

    /// <summary>Selected profile indicator (filled circle).</summary>
    ...
    /// <summary>Profile button configuration (count + default glyphs).</summary>
    ...
    /// <summary>Night-light curve color: settings override -> <see cref="EnvironmentalNightLightCurve"/>.</summary>
    ...
    /// <summary>Current-time marker color: settings override -> <see cref="EnvironmentalCurrentTime"/>.</summary>
    ...
    {
        return settings?.EnvironmentalTwilightBackdropColor.Resolve(isLightTheme)
            ?? EnvironmentalTwilightBackdrop.For(isLightTheme);
    }

    /// <summary>
    /// Night backdrop band: user RGB override (or default) combined with the user's alpha textbox value.
    /// </summary>
    public Color ResolveEnvironmentalNightBackdrop(AppSettings? settings, bool isLightTheme)
    {
        return settings?.EnvironmentalNightBackdropColor.Resolve(isLightTheme)
            ?? EnvironmentalNightBackdrop.For(isLightTheme);
    }

    /// <summary>Grid line color: settings override -> <see cref="EnvironmentalGridLine"/>.</summary>
