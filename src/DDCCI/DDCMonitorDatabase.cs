...
///
/// The backing JSON is embedded as a resource at <c>BrightnessTrayAppWPF.DDCCI.monitor-database.json</c>
...
{
    private const string ResourceName = "BrightnessTrayAppWPF.DDCCI.monitor-database.json";
    ...
    private static ParsedProfile? ParseMonitor(JsonElement m)
    {
        ...
        string edidId = ReadString(m, "edidId");
        if (string.IsNullOrEmpty(edidId)) return null;
