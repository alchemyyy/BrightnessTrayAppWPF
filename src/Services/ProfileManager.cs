...
public sealed class ProfileManager
{
    ...
    /// <summary>
    /// Invoked by callers that mutate the profile list shape directly (currently the Profiles tab's
    ...
    public void ApplyCurrentProfile(IList<MonitorInfo> monitors, bool includeBrightness)
    {
        ...
        using (DeferMonitorNotifications(monitors)) ApplyProfile(Profiles.Profiles[SelectedIndex], monitors, includeBrightness);
    }
...
#pragma warning restore CS0618
        ...
        foreach (MonitorState state in profile.MonitorStates)
        {
            ...
            if (monitor != null)
            {
                ...
                monitor.SliderState = state.IsSliderEnabled
                    ? SliderStateMachine.OnUserToggleOn(monitor.SliderState, curveEngaged: false, inDisabledPeriod: false)
    ...
    private ProfileCollection LoadOrCreate()
    {
        try
        {
            ...
            {
                using FileStream stream = new(_profilesPath, FileMode.Open);
                XmlSerializer serializer = new(typeof(ProfileCollection));
                ProfileCollection loaded = (ProfileCollection?)serializer.Deserialize(stream) ?? CreateDefault();
    ...
    /// because <c>App.xaml.cs</c> creates this manager parameterless and so the resolver can only see
    ...
    /// Run on every load and after any profile-creation path so the on-disk XmlSerializer accumulation bug
    ...
    /// (e.g. the Environmental tab's curve editor,
