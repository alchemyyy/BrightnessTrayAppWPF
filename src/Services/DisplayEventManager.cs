...
/// monitor hot-plug/unplug WM_DEVICECHANGE events. Each tick consults SetupAPI (<c>GUID_DEVCLASS_MONITOR</c>)
...
public sealed class DisplayEventManager : IDisposable
{
    ...
    public void Start()
    {
        ...
        _window.Initialize(OnMessage);

        _devNotify = DeviceNotification.RegisterForMonitorEvents(
            _window.Handle,
            "DisplayEventManager",
    ...
    {
        if (msg.Msg != DeviceNotification.WM_DEVICECHANGE) return;

        int deviceEvent = msg.WParam.ToInt32();
        if (deviceEvent is not DeviceNotification.DBT_DEVICEARRIVAL and
            not DeviceNotification.DBT_DEVICEREMOVECOMPLETE)
            return;

        // We registered only for GUID_DEVINTERFACE_MONITOR, so every notification we receive here is already
    ...
    private void Kick()
    {
        ...
        {
            _dispatcher.BeginInvoke(new Action(Kick));
            return;
        }
    ...
    private List<(string Id, string EdidKey)>? ReadSelectedProfileIdentities()
    {
        ...
        try
        {
            ...
            if (serializer.Deserialize(fs) is not ProfileCollection collection) return null;
...
#pragma warning disable CS0618
            return [.. collection.Profiles[idx].MonitorStates
...
#pragma warning restore CS0618
    ...
    {
        HashSet<string> result = new(StringComparer.OrdinalIgnoreCase);
        Guid classGuid = SetupAPI.GUID_DEVCLASS_MONITOR;
        ...
        try
        {
            SetupAPI.SP_DEVINFO_DATA devInfo = new()
            {
