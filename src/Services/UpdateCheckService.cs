...
/// <summary>
/// Background poller that asks GitHub for the latest non-prerelease of alchemyyy/BrightnessTrayAppWPF,
...
public sealed class UpdateCheckService : IDisposable
{
    ...
    private const string ReleasesApiUrl =
        "https://api.github.com/repos/alchemyyy/BrightnessTrayAppWPF/releases/latest";
    private const string UserAgent = "BrightnessTrayAppWPF-Updater";
    ...
    private const string AssetName = "BrightnessTrayAppWPF.exe";
    ...
    public async Task<bool> DownloadAndStageAsync(UpdateInfo info, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            string stagedExe = Path.Combine(
                tempRoot, $"BrightnessTrayAppWPF_update_{info.Version}_{Guid.NewGuid():N}.exe");
            string scriptPath = Path.Combine(
                tempRoot, $"BrightnessTrayAppWPF_update_{info.Version}_{Guid.NewGuid():N}.bat");
