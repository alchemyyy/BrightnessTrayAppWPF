...
{
    public const string InstalledExecutableFileName = "BrightnessTrayAppWPF.exe";
        ...
        WindowsUninstallRegistry.Scope regScope, string currentPath, int currentVersion)
    {
        ...
        bool running = string.Equals(currentPath, NormalizePath(installExecutable), StringComparison.OrdinalIgnoreCase);
        if (running)
    ...
    public static Process? RunUninstall(InstallScope scope, bool deleteSettings)
    {
        ...
        string installExecutable = NormalizePath(Path.Combine(installDir, InstalledExecutableFileName));
