...
public sealed class EnvironmentalCurveService : IDisposable
{
    ...
    public void Start()
    {
        ...
        if (wantTimer)
        {
            if (_curveTimer == null)
            {
                _curveTimer = new DispatcherTimer(DispatcherPriority.Background)
                {
                    Interval = interval,
                };
    ...
    ///
    /// Throttled to <see cref="AppSettings.BrightnessUpdateRateMs"/> on a trailing edge: the first request
    /// schedules a one-shot fire at +rateMs, and any subsequent requests during that window fold into the
    /// pending fire. After the timer fires we read the curve once - so a curve-point drag at 60Hz collapses to
    /// one re-eval per slider rate (default 50ms = 20Hz), matching the cadence the slider's own DDC writes use.
    /// </summary>
    public void RequestEvaluation()
    {
        ...
        {
            _curveEventThrottleTimer = new DispatcherTimer(DispatcherPriority.Background);
            _curveEventThrottleTimer.Tick += OnCurveEventThrottleTimerTick;
        }

        if (_curveEventThrottleTimer.IsEnabled) return;
        ...
        _curveEventThrottleTimer.Interval = TimeSpan.FromMilliseconds(rateMs);
        _curveEventThrottleTimer.Start();
    }

    private void OnCurveEventThrottleTimerTick(object? sender, EventArgs e)
    {
        _curveEventThrottleTimer?.Stop();
        Evaluate();
    }
    ...
    /// </summary>
    private void ResyncBrightnessHardwareToSliderForSleeping()
    {
    ...
    /// </summary>
    private void ResyncNightLightHardwareToSliderForSleeping()
    {
        ...
        NightLightProvider.SetStrength(sliderStrength, persistAsLastUserValue: false);
    }
    ...
    {
        int ms = _appSettings?.EnvironmentalCurveTickIntervalMs ?? TimeConstants.EnvironmentalCurveTickIntervalDefaultMs;
    ...
    private EnvironmentalCurve? ResolveLiveCurve()
    {
        ...
        if (fromLat == 0 && fromLon == 0) { fromLat = lat; fromLon = lon; }
    ...
    private void ApplyBrightnessCurve(
        EnvironmentalCurve curve, double t, double smoothness, bool absolute)
    {
        ApplyCurveCore(
            curve.Brightness,
        ...
        EnvironmentalCurve curve, double t, double smoothness, bool absolute)
    {
        ApplyCurveCore(
            ...
            applyOffsetPercent: strengthOffsetPercent =>
            {
                ...
                NightLightProvider.SetStrength(targetStrength, persistAsLastUserValue: false);
            });
    }
