...
/// <summary>
/// About page. Owns the build/runtime info rows, the Github hyperlink, the known-issues notes,
/// and the auto-update section (three setting toggles, a check-interval spinner, and a card
/// containing the manual "Check for updates" + "Install update" actions).
...
public partial class AboutPage : UserControl
{
    ...
    private void CheckForUpdates_Changed(object sender, RoutedEventArgs e)
    {
        ...
        _settings.CheckForUpdatesEnabled = CheckForUpdatesToggle.IsChecked == true;
        SaveAndNotify();
        RefreshUpdateUi();
    }

    private void ShowUpdateNotifications_Changed(object sender, RoutedEventArgs e)
    {
        if (_suppressChangeEvents || _settings == null) return;
        _settings.ShowUpdateNotificationsEnabled = ShowUpdateNotificationsToggle.IsChecked == true;
        SaveAndNotify();
    }

    private void ShowUpdateButton_Changed(object sender, RoutedEventArgs e)
    {
        if (_suppressChangeEvents || _settings == null) return;
        _settings.ShowUpdateButtonInFlyout = ShowUpdateButtonToggle.IsChecked == true;
        SaveAndNotify();
    }
