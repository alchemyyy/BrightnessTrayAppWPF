...
/// monitor-off rows get a header-on-top layout
/// (no description brick, target dropdown leftmost),
/// everything else uses the standard label-on-left layout.
/// </summary>
public sealed class HotkeyRowTemplateSelector : DataTemplateSelector
{
    public DataTemplate? FixedTemplate { get; set; }
    public DataTemplate? MonitorOffTemplate { get; set; }

    public override DataTemplate? SelectTemplate(object? item, DependencyObject container) =>
        item is HotkeyRowViewModel { ShowsTarget: true } ? MonitorOffTemplate : FixedTemplate;
}
...
/// HotkeysPage drives Entries adds/removes and re-persists into <see cref="AppSettings.Hotkeys"/>.
/// </summary>
    ...
    : INotifyPropertyChanged
{
    ...
    // \uE7BA = Segoe Fluent Icons "Warning" glyph. Empty string for non-conflict states keeps the
    // entry card visually quiet when the binding is registered or unbound.
    public string StatusGlyph => _status == HotkeyStatus.Conflict ? "\uE7BA" : string.Empty;
...
/// <summary>One entry in the per-monitor target dropdown for the "Power off specific monitor" rows.</summary>
public sealed class MonitorTargetOption
{
    public string Label { get; init; } = "";
    /// <summary>Raw target string written to <see cref="HotkeyBinding.Parameter"/> ("#1" or "edid:...").</summary>
