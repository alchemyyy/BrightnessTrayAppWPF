...
internal static class PDBSymbolResolver
{
        ...
        IReadOnlyList<string> symbolNames, Dictionary<string, int> rvas)
    {
        ...
        try
        {
            ...
            root = JsonSerializer.Deserialize<CacheRoot>(json);
        }
        catch (Exception ex)
        {
        ...
        Dictionary<string, int> rvas)
    {
        ...
        if (File.Exists(CacheFile))
        {
            try
            {
                root = JsonSerializer.Deserialize<CacheRoot>(File.ReadAllText(CacheFile)) ?? new CacheRoot();
            }
            catch
            {
                // Corrupt cache: rebuild from scratch rather than fail the write.
                root = new CacheRoot();
            }
        }
        root.Entries ??= [];
