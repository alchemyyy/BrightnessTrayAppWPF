...
internal static class NightLightRegistry
{
    ...
    private static System.Threading.Timer? _resendTimer;
    ...
    private readonly record struct OuterLayout(
        int TimestampStart, int TimestampLength, int InnerStart, int InnerLength, int TailStart)
    {
    ...
    private static int FindTagValueStart(byte[] blob, OuterLayout layout, byte[] tag)
    {
        ...
        for (int i = layout.InnerStart + OuterMagic.Length; i <= end; i++)
            if (MatchesAt(blob, i, tag)) return i + tag.Length;
    ...
    private static byte[] RebuildOuter(byte[] original, OuterLayout layout, byte[] newInner)
    {
        ...
        Array.Copy(original, 0, result, p, 10); p += 10;
        Array.Copy(freshTs, 0, result, p, freshTs.Length); p += freshTs.Length;
        Array.Copy(OuterInnerPrefix, 0, result, p, OuterInnerPrefix.Length); p += OuterInnerPrefix.Length;
        result[p++] = (byte)newInner.Length;
        Array.Copy(newInner, 0, result, p, newInner.Length); p += newInner.Length;
